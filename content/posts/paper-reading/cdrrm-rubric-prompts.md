...
把 CDRRM 与直接提示生成 Rubric 的方法（RAR 路线）放在一起，差异非常直观：

| 维度 | RAR：直接生成提示 | CDRRM：对比生成 |
| --- | --- | --- |
...
一句话总结：RAR 靠"一步枚举差异"直接给出 Rubric，速度快，但规则来自模型先验与表面差异，既不可验证，也无法保证能复现真实偏好；CDRRM 改为"先逐维取证、再合成"的两步路线，每条规则都有（分析、指令约束、响应片段）三元组背书，并经一致性过滤验证，最后还能蒸馏成 8B 生成器 + 8B 判官的轻量部署形态。

> 注：此处 RAR 指"直接提示（Direct Prompting）生成 Rubric"的方法路线，对应论文中 OpenRubrics / RUBRIC-RM 一类基线。
